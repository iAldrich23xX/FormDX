...
void CVisuals::Render()
{
	...
	if (g_Config.g_Visuals.bDMGInformer)
	{
		for (auto i = 0; i < _countof(g_DMGInformer); i++)
		{
			...
			{
				CVector vecScreen;
				g_DMGInformer[i].vecPos.z += 0.007f;
...
void CVisuals::ESP_NameTags(int iPlayerID, CPed* pPed)
{
	...
	if (g_Config.g_Visuals.bWeaponESP)
	{
		...
		RECT rect = { (LONG)vecHeadScreen.x + 20, (LONG)vecHeadScreen.y + 16, (LONG)vecHeadScreen.x + 1, (LONG)vecHeadScreen.y + 1 };
...
void CVisuals::ESP_Snipeline(int iPlayerID, CPed* pPed)
{
	...
	g_Config.g_Visuals.iSnipelineStyle ? vecTarget.z -= 1.1f : vecTarget.z += 1.0f;
...
void CVisuals::ESP_Box(int iPlayerID, CPed* pPed)
{
	...
	{
		float fWidth = abs((vecFootScreen.y - vecHeadScreen.y) / 4.f);
		...
		if (g_Config.g_Visuals.bFilledBox)
			pRender->DrawRectFilled({ vecHeadScreen.x - fWidth, vecHeadScreen.y, 0 }, { vecFootScreen.x + fWidth, vecFootScreen.y, 0 }, ImColor(colorVisible.Value.x, colorVisible.Value.y, colorVisible.Value.z, 0.35f));
		pRender->DrawRect({ vecHeadScreen.x - fWidth, vecHeadScreen.y, 0 }, { vecFootScreen.x + fWidth, vecFootScreen.y, 0 }, colorESP, g_Config.g_Visuals.fESPThickness);
	}
	else
	{
		...
		CVector vecCorner[8] =
		{
			...
			{ vecMin.x, vecMin.y * -1.f, vecMin.z },
			{ vecMin.x * -1.f, vecMin.y * -1.f, vecMin.z },
			{ vecMin.x * -1.f, vecMin.y, vecMin.z },
			{ vecMax.x * -1.f, vecMax.y * -1.f, vecMax.z },
			{ vecMax.x * -1.f, vecMax.y, vecMax.z },
			{ vecMax.x, vecMax.y, vecMax.z },
			{ vecMax.x, vecMax.y * -1.f, vecMax.z }
		};

		for (auto i = 0; i < 8; i++)
			plugin::Command<plugin::Commands::GET_OFFSET_FROM_CHAR_IN_WORLD_COORDS>(pPed, vecCorner[i].x, vecCorner[i].y, vecCorner[i].z, &vecCorner[i].x, &vecCorner[i].y, &vecCorner[i].z);
...
void CVisuals::ESP_Vehicle()
{
	for (int i = 0; i <= SAMP_MAX_VEHICLES; i++)
	{
		...
		if (g_Config.g_Visuals.bVehicleNameTagsESP)
		{
			...
			vecVehicleScreen.y -= 14.f;
			vecVehicleScreen.y = pRender->DrawString(pRender->pESPFont, szName, vecVehicleScreen, 14.f, 0xFFFFFFFF, true);
		}
		...
		if (g_Config.g_Visuals.bVehicleHPESP)
		{
			...
			vecVehicleScreen.y = pRender->DrawString(pRender->pESPFont, szHealth, vecVehicleScreen, 14.f, 0xFFFFFFFF, true);
		}

		if (g_Config.g_Visuals.bVehicleEngineESP)
			vecVehicleScreen.y = pRender->DrawString(pRender->pESPFont, pSAMP->getVehicles()->pSAMP_Vehicle[i]->bIsMotorOn ? "Engine: OFF" : "Engine: ON", vecVehicleScreen, 14.f, 0xFFFFFFFF, true);

		if (g_Config.g_Visuals.bVehicleStatusESP)
			vecVehicleScreen.y = pRender->DrawString(pRender->pESPFont, pSAMP->getVehicles()->pSAMP_Vehicle[i]->iIsLocked ? "Status: Locked" : "Status: Unlocked", vecVehicleScreen, 14.f, 0xFFFFFFFF, true);

		if (g_Config.g_Visuals.bVehicleDistanceESP)
		{
			char szDistance[64];
			sprintf(szDistance, "Distance: %.2f m", Utils::getDistance(vecVehicle));
			vecVehicleScreen.y = pRender->DrawString(pRender->pESPFont, szDistance, vecVehicleScreen, 14.f, 0xFFFFFFFF, true);
		}
		...
		if (g_Config.g_Visuals.bVehicleBoundingBoxESP)
		{
			...
			for (auto i = 0; i < 8; i++)
				plugin::Command<plugin::Commands::GET_OFFSET_FROM_CAR_IN_WORLD_COORDS>(pVehicle, vecCorner[i].x, vecCorner[i].y, vecCorner[i].z, &vecCorner[i].x, &vecCorner[i].y, &vecCorner[i].z);
